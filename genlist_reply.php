...
//SmiliesDialog.insert
$smilies_sum = count($clcs_smilies);
$smilies_counter = 0;
$smilies_col = 5;
$smilies_row = ceil($smilies_sum/5);
$smilies_space = $smilies_row * $smilies_col - $smilies_sum;
$clcs_smilies_list = '<div id="smilieslist" class="flora" title="Smilies" style="display: none;"><table border="0" cellspacing="0" cellpadding="4">'; ?>
<?php foreach ($clcs_smilies as $k => $v) : ?>
<?php if ($smilies_count % $smilies_col == 0) { ?>
<?php $clcs_smilies_list .= '	<tr>'; ?>
<?php } ?>
<?php $clcs_smilies_list .= '		<td><a href="javascript:smilies_insert(\\\'' . $k . '\\\');"><img src="' . $imagesdirurl . $v .'" border="0" alt="smilies" title="smilies" /></a>&nbsp;</td>'; ?>
<?php
if ($smilies_count >= $smilies_sum - 1) {
	for ($i = 0; $i < $smilies_space; $i++) {
		$clcs_smilies_list .= '		<td></td>';
		$smilies_count++;
	}
}
?>
...
<?php $smilies_count++; ?>
<?php endforeach; ?>
<?php $clcs_smilies_list .= '</table></div>'; ?>